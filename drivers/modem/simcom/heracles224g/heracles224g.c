...
LOG_MODULE_REGISTER(modem_simcom_heracles224g, CONFIG_MODEM_LOG_LEVEL);
...
static struct heracles224g_data mdata;
...
static struct heracles224g_gnss_data gnss_data;

static K_KERNEL_STACK_DEFINE(modem_rx_stack, CONFIG_MODEM_SIMCOM_HERACLES224G_RX_STACK_SIZE);
static K_KERNEL_STACK_DEFINE(modem_workq_stack, CONFIG_MODEM_SIMCOM_HERACLES224G_RX_WORKQ_STACK_SIZE);
...
/**
 * Changes the operating state of the heracles224g.
 *
 * @param state The new state.
 */
static void change_state(enum heracles224g_state state)
{
...
/**
 * Get the current operating state of the heracles224g.
 *
 * @return The current state.
 */
static enum heracles224g_state get_state(void)
{
	return mdata.state;
}

/*
 * Parses the +CIPSTART command and gives back the
 * connect semaphore.
 */
...
static int offload_connect(void *obj, const struct sockaddr *addr, socklen_t addrlen)
{
	...
	uint16_t dst_port = 0;
	char *protocol;
	struct modem_cmd cmd[] = { MODEM_CMD("+CIPSTART: ", on_cmd_caopen, 2U, ",") };
	char buf[sizeof("AT+CIPSTART: #,#,#####,#xxxx:xxxx:xxxx:xxxx:xxxx:xxxx:xxxx:xxxx#,####")];
	char ip_str[NET_IPV6_ADDR_LEN];
	int ret;

	/* Modem is not attached to the network. */
	if (get_state() != HERACLES224G_STATE_NETWORKING) {
		return -EAGAIN;
	}
	...
	ret = snprintk(buf, sizeof(buf), "AT+CIPSTART=%d,%d,\"%s\",\"%s\",%d", 0, sock->id,
 ...
 * This is done by sending AT+CIPSEND=<sockfd>,<nbytes>\r\n.
			      ...
			      const struct sockaddr *dest_addr, socklen_t addrlen)
{
	...
	ret = snprintk(send_buf, sizeof(send_buf), "AT+CIPSEND=%d,%ld", sock->id, (long)len);
	if (ret < 0) {
...
MODEM_CMD_DEFINE(on_urc_ftpget)
{
	...
	/* Transfer finished. */
	if (error == 0) {
		mdata.ftp.state = HERACLES224G_FTP_CONNECTION_STATE_FINISHED;
	} else if (error == 1) {
		mdata.ftp.state = HERACLES224G_FTP_CONNECTION_STATE_CONNECTED;
	} else {
		mdata.ftp.state = HERACLES224G_FTP_CONNECTION_STATE_ERROR;
	}
...
MODEM_CMD_DEFINE(on_cmd_cgpaddr)
{
	strcpy(mdata.mdm_ip_addr, argv[1]);
	LOG_INF("CGPADDR: %s", mdata.mdm_ip_addr);
	return 0;
}
...
/*
 * Possible responses by the heracles224g.
 */
...
static int modem_pdp_activate(void)
{
	...
	const char *apn = CONFIG_MODEM_CELLULAR_APN;
	char pdp_buf[50] = {0};
	...
	struct modem_cmd cgpaddr_cmd[] = { MODEM_CMD("+CGPADDR: ", on_cmd_cgpaddr, 2U, ",") };
	...
	LOG_INF("Network registration OK");

	/* Configure the PDP context (IPv4/IPv6) */
	memset(pdp_buf, '\0', sizeof(pdp_buf));
	ret = snprintk(pdp_buf, sizeof(pdp_buf), "AT+CGDCONT=%d,\"%s\",\"%s\"", 1, "IP", apn);
	LOG_INF("%s", pdp_buf);
	if (ret < 0) {
		LOG_ERR("Failed to build config PDP command. APN:%s", apn);
		errno = ENOMEM;
		return -1;
	}

	ret = modem_cmd_send(&mctx.iface, &mctx.cmd_handler, NULL, 0, pdp_buf,
		&mdata.sem_response, MDM_BASIC_CMD_TIMEOUT);
	...
	LOG_INF("Configure PDP context OK");

	/*
	 * Now activate the pdp context and wait for confirmation.
	 */
	ret = modem_cmd_send(&mctx.iface, &mctx.cmd_handler, NULL, 0, "AT+CGACT=1,1",
			     &mdata.sem_response, MDM_ACTIVATE_PDP_CONTEXT_TIMEOUT);
	if (ret < 0) {
		LOG_ERR("Could not activate PDP context.");
		goto error;
	}

	LOG_INF("Activate PDP context OK");

	// context authentification: AT+CSTT
	memset(pdp_buf, '\0', sizeof(pdp_buf));
	ret = snprintk(pdp_buf, sizeof(pdp_buf), "AT+CSTT=\"%s\"", apn);
	LOG_INF("%s", pdp_buf);
	ret = modem_cmd_send(&mctx.iface, &mctx.cmd_handler, NULL, 0, pdp_buf,
		&mdata.sem_response, MDM_BASIC_CMD_TIMEOUT);
	if (ret < 0) {
		LOG_ERR("Could not authentificate context.");
		goto error;
	}

	LOG_INF("Start PDP context OK");

	// bring up wireless connection
	ret = modem_cmd_send(&mctx.iface, &mctx.cmd_handler, NULL, 0, "AT+CIICR",
		&mdata.sem_response, MDM_BRING_UP_WIRELESS_CONNECTION);
	if (ret < 0) {
		LOG_ERR("Could not bring up wireless connection.");
		goto error;
	}

	LOG_INF("Bring up wireless connection OK");

	// get the local IP address
	ret = modem_cmd_send(&mctx.iface, &mctx.cmd_handler, cgpaddr_cmd,
		ARRAY_SIZE(cgpaddr_cmd), "AT+CGPADDR=1", &mdata.sem_response,
		MDM_BASIC_CMD_TIMEOUT);

	if (ret < 0) {
		LOG_ERR("Could not get local ip address.");
		goto error;
	}

	LOG_INF("Get local IP addr OK.");
...
#endif /* defined(CONFIG_MODEM_SIM_NUMBERS) */
#if defined(CONFIG_MODEM_SIMCOM_HERACLES224G_RAT_NB1)
	SETUP_CMD_NOHANDLE("AT+CNMP=38"),
	SETUP_CMD_NOHANDLE("AT+CMNB=2"),
	SETUP_CMD_NOHANDLE("AT+CBANDCFG=\"NB-IOT\"," MDM_LTE_BANDS),
#endif /* defined(CONFIG_MODEM_SIMCOM_HERACLES224G_RAT_NB1) */
#if defined(CONFIG_MODEM_SIMCOM_HERACLES224G_RAT_M1)
	SETUP_CMD_NOHANDLE("AT+CNMP=38"),
	SETUP_CMD_NOHANDLE("AT+CMNB=1"),
	SETUP_CMD_NOHANDLE("AT+CBANDCFG=\"CAT-M\"," MDM_LTE_BANDS),
#endif /* defined(CONFIG_MODEM_SIMCOM_HERACLES224G_RAT_M1) */
#if defined(CONFIG_MODEM_SIMCOM_HERACLES224G_RAT_GSM)
	SETUP_CMD_NOHANDLE("AT+CNMP=13"),
#endif /* defined(CONFIG_MODEM_SIMCOM_HERACLES224G_RAT_GSM) */
...
static int modem_autobaud(void)
{
	...
	while (boot_tries++ <= MDM_BOOT_TRIES) {
		...
		/*
		 * The heracles224g has a autobaud function.
...
int mdm_heracles224g_query_gnss(struct heracles224g_gnss_data *data)
{
	...
	if (get_state() != HERACLES224G_STATE_GNSS) {
...
int mdm_heracles224g_start_gnss(void)
{
	...
	change_state(HERACLES224G_STATE_INIT);
	...
	change_state(HERACLES224G_STATE_GNSS);
	return 0;
}
...
int mdm_heracles224g_ftp_get_read(char *dst, size_t *size)
{
	...
	/* Some error occurred. */
	if (mdata.ftp.state == HERACLES224G_FTP_CONNECTION_STATE_ERROR ||
	    mdata.ftp.state == HERACLES224G_FTP_CONNECTION_STATE_INITIAL) {
		return HERACLES224G_FTP_RC_ERROR;
	}
	...
	if (mdata.ftp.state == HERACLES224G_FTP_CONNECTION_STATE_FINISHED) {
		*size = 0;
		return HERACLES224G_FTP_RC_FINISHED;
	} else if (mdata.ftp.state == HERACLES224G_FTP_CONNECTION_STATE_ERROR) {
		*size = 0;
		return HERACLES224G_FTP_RC_ERROR;
	}
			      ...
			      const char *file, const char *path)
{
	...
	/* Start network. */
	ret = mdm_heracles224g_start_network();
	if (ret < 0) {
	...
	if (mdata.ftp.state != HERACLES224G_FTP_CONNECTION_STATE_CONNECTED) {
 ...
 */
static int mdm_decode_pdu(const char *pdu, size_t pdu_len, struct heracles224g_sms *target_buf)
{
	size_t index;
...
static int modem_setup(void)
{
	...
	change_state(HERACLES224G_STATE_NETWORKING);
...
int mdm_heracles224g_power_off(void)
{
	...
	ret = modem_cmd_send(&mctx.iface, &mctx.cmd_handler, NULL, 0U, "AT", &mdata.sem_response,
			     K_MSEC(1000));
	if (ret < 0) {
		change_state(HERACLES224G_STATE_OFF);
		return 0;
	}
...
NET_DEVICE_DT_INST_OFFLOAD_DEFINE(0, modem_init, NULL, &mdata, NULL,
				  CONFIG_MODEM_SIMCOM_HERACLES224G_INIT_PRIORITY, &api_funcs,
				  MDM_MAX_DATA_LENGTH);

NET_SOCKET_OFFLOAD_REGISTER(simcom_heracles224g, CONFIG_NET_SOCKETS_OFFLOAD_PRIORITY,
